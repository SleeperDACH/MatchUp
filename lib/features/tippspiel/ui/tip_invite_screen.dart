...
/// „Mitglieder einladen" für eine Tipprunde: zeigt eigene Chats + Freunde (und
/// eine Suche) und verschickt eine tippbare Beitreten-Einladung als
/// Direktnachricht — analog zur Fantasy-Einladung.
class TipInvitePlayersScreen extends ConsumerStatefulWidget {
  const TipInvitePlayersScreen({super.key, required this.round});

  final TipRound round;

  @override
  ConsumerState<TipInvitePlayersScreen> createState() =>
      _TipInvitePlayersScreenState();
}

class _TipInvitePlayersScreenState
    extends ConsumerState<TipInvitePlayersScreen> {
  ...
  Future<void> _invite(String userId) async {
    ...
    try {
      await ref.read(messagingRepositoryProvider).sendMessage(
            userId,
            '🎯 Einladung zum Tippspiel „${r.name}" — tritt direkt bei!',
            inviteLeagueId: r.id,
  ...
  Widget build(BuildContext context) {
    // Bereits beigetretene Mitglieder als Einladungsziel ausschließen.
    final memberIds = <String>{
      ...?ref
          .watch(roundMembersProvider(widget.round.id))
          .valueOrNull
          ?.map((m) => m.userId),
    };
    ...
    // Offene Beitrittsanfragen (öffentlich–auf Einladung) für den Admin.
    final pending =
        ref.watch(tipJoinRequestsProvider(widget.round.id)).valueOrNull ??
            const [];

    return Scaffold(
      appBar:
          AppBar(centerTitle: true, title: const Text('Mitglieder einladen')),
      body: Column(
        children: [
