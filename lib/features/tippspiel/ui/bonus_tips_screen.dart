          ...
          if (teams.isEmpty) {
            return const Center(
              child: Padding(
                ...
                child: Text(
                  'Noch keine Teams für diesen Wettbewerb geladen.',
    ...
    try {
      ...
      for (final q in widget.round.scoring.bonusTips) {
        await repo.setBonusAnswers(
          ...
          question: q,
          teams: [
            for (final t in _picks[q] ?? const []) (id: t.id, name: t.name),
          ],
        );
      }
...
class _DeadlineBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // **Hauch statt Farbfläche mit farbigem Rand.** Der Balken trug beides
    // und war damit das lauteste Element des Schirms — für einen Hinweis, der
    // nur eine Frist nennt.
    return Karte(
      hauch: color,
      ...
      child: Row(
        children: [
          Icon(
            open ? Icons.timelapse : Icons.lock_outline,
            size: 18,
            color: color,
          ),
