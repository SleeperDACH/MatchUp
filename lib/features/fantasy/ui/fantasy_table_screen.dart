...
/// Liga-Tabelle: kumulative Saison-Gesamtpunkte je Manager (Summe der
/// effektiven Startelf über alle gespielten Spieltage), absteigend sortiert.
...
class FantasyTableBody extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    for (final m in managers) {
      ...
      var total = 0;
      seasonStats.forEach((round, stats) {
        final points = {
          for (final p in players)
            p: scorePlayer(stats[p.id] ?? const PlayerMatchStats(), p.position,
                league.scoring)
        };
      ...
      rows.add((name: m.username, me: m.userId == myId, total: total));
    }
    ...
    return ListView(
      children: [
        ...
        Padding(
          ...
          child: Text(
            'Gesamtpunkte über alle Spieltage = gewählte Startelf (sonst '
            'automatisch die beste Elf). Wertung aus OpenLigaDB (Tore, Zu-Null).',
